...
/* Reads a variable length string through a single register */
int read_string_register(modbus_t *ctx, int function, int addr, int nb,
	char *dest)
{
	int rc;
	int req_length;
	...
	uint8_t rsp[MAX_MESSAGE_LENGTH];
#if defined(_DEBUG)
	uint8_t debug_dest[MAX_MESSAGE_LENGTH];
#endif
	...
	if (rc > 0) {
		...
		if (rc == -1)
			return -1;
		rc = ctx->backend->check_integrity(ctx, rsp, rc);
/*		rc = check_confirmation(ctx, req, rsp, rc); */
		if (rc == -1)
			return -1;
		...
		for (i = 0; (i < rsp[offset+1]) && (i < sizeof(rsp)/sizeof(uint8_t)); i++)
		{
			dest[i] = (rsp[offset+i+2]);
#if defined(_DEBUG)
			debug_dest[i] = dest[i];
#endif
		}
...
/* Reads a string value through a single holding register of remote device and put the data into an
array */
int modbus_read_string_register(modbus_t *ctx, int addr, int nb, char *dest)
{
	int status = 0;
	if (nb > MODBUS_MAX_READ_REGISTERS) {
		if (ctx->debug) {
	...
	return	(status = read_string_register(ctx, _FC_READ_HOLDING_REGISTERS,
		addr, nb, dest));
}
